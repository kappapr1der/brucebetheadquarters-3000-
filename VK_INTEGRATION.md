...
- Predictions remain read-only until `VK_PREDICTIONS_IMPORT_ENABLED=1` is explicitly set for the configured EPL topic. When enabled, new valid submissions, edits, rejected late edits, and quarantines enter a durable Telegram outbox; delivery is retried per Telegram whitelist chat without reimporting forecasts. Only accepted projections then refresh the separate Bruce contest-pick ledger for the affected round. Duplicates, rejected late edits and quarantines never update the field or trigger a contest-pick notification.
...
3. Set `VK_PREDICTIONS_SNAPSHOT_ENABLED=1` to archive changed prediction fields every 20 minutes by default. `/picks [тур]` renders the separate auditable Bruce contest recommendation; it does not add a prediction for Bruce.
4. The final pick dispatcher checks once per minute and freezes a pre-deadline snapshot 10 minutes before the effective deadline by default (`BRUCEBET_FINAL_PICK_LEAD_MINUTES`). If field completeness or intelligence is missing, it is marked provisional instead of final.
