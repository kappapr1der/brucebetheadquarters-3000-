...
`/vk_snapshot` reads the configured EPL registration and prediction topics, keeps a local archive of changed public fields in `data/vk_snapshots/`, and reports the recognized entry/block counts. Set `VK_PREDICTIONS_SNAPSHOT_ENABLED=1` to run the same read-only archive job every 20 minutes. VK is always read-only. SQLite projection is a separate explicit gate: `VK_PREDICTIONS_IMPORT_ENABLED=1` imports only the configured EPL topic and maps fixtures by stable home/away identity rather than VK list position.

Для темы прогнозов reader начинает с обычного публичного URL и переходит только по server-side pagination URL, которые реально встретились в DOM этой же темы. Он не угадывает `offset` или cursor. `VK_TOPIC_MAX_PAGES` ограничивает обход; повтор страницы, цикл, ошибка на последующей странице, client-side `load more` без безопасной ссылки, несколько comment permalink без pagination-навигации или VK challenge дают статус `incomplete`. Такой снимок не импортируется, не может перевести поле в `ready/final` и создаёт одно дедуплицированное Telegram-предупреждение. `vk_capture_state.csv` содержит безопасные метаданные: страницы, stop reason, покрытые туры, диапазон времён комментариев и время последнего полного снимка.
...
Historical recovery is an explicit one-round operation on a complete capture and a database copy. It uses the original VK timestamp for unseen comments, keeps genuinely late forecasts rejected, refreshes competitor scoring and the completed-round review, and sends one recovery summary instead of one alert per old participant. It never recomputes or rewrites frozen `contest_recommendations`; those preserve what BruceBet actually knew at the old deadline.
